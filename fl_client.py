...
class ShenzhenClientDataset(Dataset):
    """Dataset for a federated Shenzhen client loaded from a CSV partition."""

    def __init__(self, csv_file: Path, transform=None):
        df = pd.read_csv(csv_file)
        if 'path' not in df.columns:
            raise ValueError(f"Partition file '{csv_file}' must contain a 'path' column.")

        if 'label' not in df.columns:
            df['label'] = df['path'].apply(infer_label_from_path)

        self.dataframe = df.copy().reset_index(drop=True)
        self.dataframe['path'] = self.dataframe['path'].astype(str)
...
class ShenzhenFlowerClient(fl.client.NumPyClient):
    """Flower client tailored for the Shenzhen tuberculosis dataset."""
    ...
    def __init__(self, client_id, partition_path, valid_csv, device, batch_size=32, local_epochs=1):
        ...
        train_dataset = ShenzhenClientDataset(partition_path, transform=train_transforms)
        valid_dataset = ShenzhenClientDataset(valid_csv, transform=valid_transforms)
        ...
        print(
            f"Client {client_id}: Training samples = {len(train_dataset)}, "
...
def main():
    parser = argparse.ArgumentParser(description='Flower client for Shenzhen TB dataset')
    ...
    partition_path = partitions_dir / args.partition_strategy / f'client_{args.client_id}_images.csv'
