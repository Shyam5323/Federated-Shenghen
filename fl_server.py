...
from flwr.common import FitRes, Metrics, Parameters, ndarrays_to_parameters, parameters_to_ndarrays
...
def main():
    parser = argparse.ArgumentParser(description='Flower server for Shenzhen TB dataset')
    ...
    parser.add_argument('--history_dir', type=str, default='history',
                        help='Directory to write training history JSON files')
    ...
    if args.save_path is None:
        ...
        args.save_path = str(models_dir / f"best_federated_shenzhen_{args.partition_strategy}.pth")
    ...
    # Persist history if available
    if history is not None:
        history_dir = Path(args.history_dir)
        history_dir.mkdir(parents=True, exist_ok=True)
        auc_history = history.metrics_distributed.get("auc", [])
        history_payload = [
            {"round": int(round_idx), "auc": float(value)}
            for round_idx, value in auc_history
        ]
        history_path = history_dir / f"history_{args.partition_strategy}.json"
        with history_path.open("w", encoding="utf-8") as fp:
            json.dump(history_payload, fp, indent=4)
