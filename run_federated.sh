...
mkdir -p "$LOG_DIR"
SERVER_LOG="$LOG_DIR/server_${STRATEGY}.log"
CLIENT_LOG="$LOG_DIR/clients_${STRATEGY}.log"

# Truncate previous logs for this strategy
: > "$SERVER_LOG"
: > "$CLIENT_LOG"
...
python fl_server.py \
    ...
    --history_dir history >> "$SERVER_LOG" 2>&1 &
...
for i in $(seq 0 $((NUM_CLIENTS-1))); do
    ...
    python fl_client.py \
        ...
        --local_epochs $LOCAL_EPOCHS >> "$CLIENT_LOG" 2>&1 &
